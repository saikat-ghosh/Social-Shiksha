@extends('layouts.dashboard-layout')
    ...
    @section('sidebar-menu-left')
        ...
        <!-- Left Sidebar menu goes here -->
                <ul class="list-group sidebar-list" id="sidebar">
                    ...
                    <li>
                        <a  href="{{url('/teacher/upload-test-or-practice-paper')}}" class="active list-group-item sidebar-list-item"><i class="fa fa-upload fa-2x"></i> Upload Mock Test/Practice Paper</a>
                    </li>
                    <li>
    ...
    @endsection
       ...
       @section('menu-content')
           ...
           <div id="upload-test-paper" class="row padding">
               <div class="col-md-8 col-md-offset-2">
                   <div class="panel panel-default">
                       <div class="panel panel-heading">
                           <b>Upload Mock Test/Practice Paper</b>
                       </div>
                       ...
                       <div class="panel panel-body">
                           ...
                           @else
                               <form class="form-horizontal padding" enctype="multipart/form-data" role="form" action="{{ action('TeacherController@saveUploadedMockTestOrPracticePaper') }}" method="POST">
                                   {{ csrf_field() }}
                                   <!-- Upload Assignment Here -->

                                   <input type="hidden" name ="MT_User_Id" value="{{$teacher->id}}">

                                    <div class="form-group">
                                        <label for="Batch_id" class="control-label">Select Batch</label>
                                        <div>
                                            <select id="Batch_id" name="MT_Batch_Id" class="form-control input-sm" required>
                                    ...
                                    <div class="form-group">
                                        <label for="File_Upload" class="control-label">Drop Mock Test/Practice Paper Here</label>
                                        <div>
                            ...
                            <div>
                                   @if($uploadedFiles->isEmpty())
                                       <h4>You have not uploaded any assignment papers yet.</h4>
                                   @else
                                       <h4><b>Uploaded Test/Practice Papers</b></h4>
                                       <ul class="list-group striped-list">
                                           @foreach($uploadedFiles as $file)
                                               <li class="list-group-item">
                                                   <strong>{{ $file->MT_File_Name }}</strong>&nbsp;&nbsp;
                                                   {{ $file->MT_Subject }}&nbsp;&nbsp;
                                                   {{ $file->MT_Upload_Date }}
                                                   <!-- link to delete test/practice paper -->
                                                   <a href="{{ action('TeacherController@deleteUploadedMockTestOrPracticePaper',$file->id) }}" data-toggle="tooltip" title="Delete" class="xs-small-font pull-right"
                                                      onclick="event.preventDefault();
                                                             document.getElementById('delete-assignment-form').submit();">
                                                       <span class="glyphicon glyphicon-trash"></span>
                                                   </a>
                                                   <!-- Submit this form to test/practice paper -->
                                                   <form id="delete-assignment-form" action="{{ action('TeacherController@deleteUploadedMockTestOrPracticePaper',$file->id) }}" method="POST" style="display: none;">
                                                       {{ csrf_field() }}
                                                       {{ method_field('DELETE') }}
                                                   </form>
                                               </li>
