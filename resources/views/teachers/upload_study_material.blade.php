@extends('layouts.dashboard-layout')
    ...
    @section('sidebar-menu-left')
        ...
        <!-- Left Sidebar menu goes here -->
                <ul class="list-group sidebar-list" id="sidebar">
                    ...
                    <li>
                        <a  href="{{url('/teacher/upload-study-material')}}" class="active list-group-item sidebar-list-item"><i class="fa fa-upload fa-2x"></i> Upload Study Material</a>
                    </li>
                    <li>
    ...
    @endsection
       ...
       @section('menu-content')
           ...
           <div id="upload-study-material" class="row padding">
               <div class="col-md-8 col-md-offset-2">
                   <div class="panel panel-default">
                       ...
                       <div class="panel panel-body">
                           ...
                           @else
                               <form class="form-horizontal padding" enctype="multipart/form-data" role="form" action="{{ action('TeacherController@saveUploadedStudyMaterial') }}" method="POST">
                                   {{ csrf_field() }}
                                   <!-- Upload Assignment Here -->

                                   <input type="hidden" name ="SM_User_Id" value="{{$teacher->id}}">

                                    <div class="form-group">
                                        <label for="Batch_id" class="control-label">Select Batch</label>
                                        <div>
                                            <select id="Batch_id" name="SM_Batch_Id" class="form-control input-sm" required>
                                    ...
                                    <div class="form-group">
                                        <label for="File_Upload" class="control-label">Drop Study Material Here</label>
                                        <div>
                            ...
                            <div>
                                   @if($uploadedFiles->isEmpty())
                                       <h4>You have not uploaded any study materials yet.</h4>
                                   @else
                                       <h4><b>Uploaded Study Materials</b></h4>
                                       <ul class="list-group striped-list">
                                           @foreach($uploadedFiles as $file)
                                               <li class="list-group-item">
                                                   <strong>{{ $file->SM_File_Name }}</strong>&nbsp;&nbsp;
                                                   {{ $file->SM_Subject }}&nbsp;&nbsp;
                                                   {{ $file->SM_Upload_Date }}
                                                   <!-- link to delete study material -->
                                                   <a href="{{ action('TeacherController@deleteUploadedStudyMaterial',$file->id) }}" data-toggle="tooltip" title="Delete" class="xs-small-font pull-right"
                                                      onclick="event.preventDefault();
                                                             document.getElementById('delete-StudyMaterial-form').submit();">
                                                       <span class="glyphicon glyphicon-trash"></span>
                                                   </a>
                                                   <!-- Submit this form to study material -->
                                                   <form id="delete-StudyMaterial-form" action="{{ action('TeacherController@deleteUploadedStudyMaterial',$file->id) }}" method="POST" style="display: none;">
                                                       {{ csrf_field() }}
                                                       {{ method_field('DELETE') }}
                                                   </form>
                                               </li>
